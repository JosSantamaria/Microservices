    ...
    public class Login
    {
        ...
        public class UsuarioLoginValidation : AbstractValidator<UsuarioLoginCommand> 
        {
            ...
            public class UsuarioLoginHandler : IRequestHandler<UsuarioLoginCommand, UsuarioDto>
            {
                ...
                public UsuarioLoginHandler(SeguridadContexto context, UserManager<Usuario> userManager, IMapper mapper, IJwtGenerator jwtGenerator, SignInManager<Usuario> singInManager) 
                {
                    _context = context;
                ...
                public async Task<UsuarioDto> Handle(UsuarioLoginCommand request, CancellationToken cancellationToken)
                {
                    ...
                    {
                      var UsuarioDto =  _mapper.Map<Usuario, UsuarioDto>(usuario); //Tomamos el usuario lo mapeamos a la clase DTO (Data Transformation Object)
                        UsuarioDto.Token = _jwtGenerator.CreateToken(usuario); //Generamos el Token del Usuario
                        return UsuarioDto; //Retornamos
                    }
